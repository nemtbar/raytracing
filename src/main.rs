...
fn frag(x: usize, y: usize, width: usize, height: usize) -> Pixel{
    ...
    let objects: Vec<Object> = vec![
        ...
        Object::Sphere {pos: Vec3::new(0., 0., 0.6), col: Vec3::new(1., 1., 1.), rad: 2., emmision: 0.3}
    ];
    let mut col = Pixel::default();
    let c: u8 = 40;
    for i in 0..c{
        if i == 0{
            col = Object::bounce(ray.clone(), &objects, 40);
        } else {
            col = Object::bounce(ray.clone(), &objects, 40).lerp(&col, 1.- 1./(c as f32));
        }
    }
    col
